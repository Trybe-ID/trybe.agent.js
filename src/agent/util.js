...
const fs = require('fs')
const os = require('os')
const testnetTxs = require('./config/genesis-txs-testnet')

async function getPoolGenesisTxnPath(network, poolName) {
    const path = `${os.tmpdir()}/indy/${poolName}.txn`
    const data = await poolGenesisTxnData(network)
    await mkdir(path)
    await fs.writeFileSync(path, data, 'utf8')
    return path
}

async function poolGenesisTxnData(network) {
    if (network === 'test') {
        ...
        `
    } else if (network === 'sovrin-builder') {
        return `
